    ...
    <div className="document-table" role="table" aria-label="Documents">
      ...
      {documents.length === 0 ? <div className="empty-row">No documents match this view.</div> : documents.map((document) => (
        <Link role="row" key={document.id} href={`/dashboard?document=${encodeURIComponent(document.slug)}`} className={document.slug === selectedSlug ? "document-row selected" : "document-row"}>
          ...
          <span>{document.collection ?? "Unfiled"}</span><span>{document.versionCount}</span><span>{dateFormatter.format(new Date(document.updatedAt))}</span><span className="sharing-state">{document.shared ? <><Share2 size={16} />Shared</> : <><LockKeyhole size={16} />Private</>}</span>
        </Link>
      ))}
    </div>
  );
...
export function DetailPanel({ document }: { document: Detail }) {
  return (
    <aside className="detail-panel" aria-label={`${document.title} details`}>
      <Link className="close-panel" href="/dashboard" aria-label="Close details"><X size={20} /></Link>
      ...
      <div className="detail-actions"><Link className="button primary" href={`/dashboard/documents/${encodeURIComponent(document.slug)}`}>Open</Link><ShareControls slug={document.slug} shared={document.shared} /></div>
...
function ShareControls({ slug, shared }: { slug: string; shared: boolean }) {
  return <Link className="button" href={`/dashboard/documents/${encodeURIComponent(slug)}#sharing`}>{shared ? "Manage share" : "Share"}</Link>;
}
