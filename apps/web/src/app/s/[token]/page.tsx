...
export default async function SharePage({ params }: Props) {
  ...
  const rawSource = createShareUrls(appOrigin(), token).contentUrl;
  return (
    <main className="flex min-h-screen flex-col bg-muted/40">
      <header className="flex min-h-16 items-center justify-between gap-4 border-b border-border/80 bg-background px-4 sm:px-6 lg:px-8"><div className="flex min-w-0 items-center gap-3"><span className="flex size-9 items-center justify-center rounded-xl bg-primary/10 text-primary"><FileCode2 /></span><div className="typeset typeset-ui min-w-0"><strong className="block truncate text-sm font-semibold">{shared.title}</strong><span className="mono-meta mt-1 block truncate text-muted-foreground">Latest version · updated {new Date(shared.updatedAt).toLocaleDateString()}</span></div><Badge variant="secondary" className="hidden rounded-lg font-normal sm:inline-flex">Public link</Badge></div><div className="flex items-center gap-2"><a href={rawSource} target="_blank" rel="noreferrer" type="text/html" data-htmlpub-content-url className={`${buttonVariants({ variant: "outline", size: "sm" })} rounded-xl`}><Code2 data-icon="inline-start" />Raw HTML</a><a href={source} target="_blank" rel="noreferrer" className={`${buttonVariants({ variant: "outline", size: "sm" })} hidden rounded-xl sm:inline-flex`}><ExternalLink data-icon="inline-start" />Fullscreen</a></div></header>
