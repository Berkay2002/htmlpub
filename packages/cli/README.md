...
`documents content` retrieves the current version through the authenticated API with `documents:read`. Markdown is the default format. Without `--output`, normal mode writes content bytes directly to stdout and `--json` returns UTF-8 content in its data envelope. Use `--output` to preserve exact response bytes in a file.
...
Give `url` to a person for the reader page. Give `markdownUrl` to an agent by default for compact GFM text. Give `contentUrl` to an agent or HTTP client that needs the authoritative HTML bytes, scripts, or interactive structure. The raw HTML URL responds directly as inert `text/plain`; the Markdown URL follows a temporary redirect to the isolated renderer. Neither requires an htmlpub API token. The reader page's Share to agent action copies a ready-to-paste prompt with the raw HTML link, artifact context, and an authenticated `documents content` fallback. Revoking or rotating sharing invalidates all three stable URLs.
